...
namespace Filesystem {
		...
		public:
			WaveWriter(const string &file, uint32_t sampleRate, uint16_t channels) throw(...)
            ...
            void writeFile(const IHostMultiChannel<SampleType> &buffer)
            {
                for(unsigned int s = 0; s < buffer.getSampleCount(); ++s)
                    for(unsigned int c = 0; c < buffer.getChannelCount(); ++c)
                        write(buffer.getRawReal(c)[s]);
            }
